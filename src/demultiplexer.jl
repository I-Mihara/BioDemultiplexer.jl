...
"""
function process_fastq_file(fastq_R1::String, fastq_R2::String, bc_df::DataFrame, output_dir::String; match=0, mismatch=-1, indel=-2, read_basis::String="R1", classify_r1=false, classify_r2=true)
    primary_file_path = read_basis == "R1" ? fastq_R1 : fastq_R2
    secondary_file_path = read_basis == "R1" ? fastq_R2 : fastq_R1
    classify_primary = read_basis == "R1" ? classify_r1 : classify_r2
    classify_secondary = read_basis == "R1" ? classify_r2 : classify_r1
 
    if classify_secondary
       if classify_primary
          mkdir(output_dir * "/R1")
          mkdir(output_dir * "/R2")
          open(primary_file_path, "r") do primary_file
             open(secondary_file_path, "r") do secondary_file
    ...
    elseif classify_primary
       open(primary_file_path, "r") do file
          header, seq, plus, quality_score = "", "", "", ""
          mode = "header"
          filename=""
...
# function process_fastq_file(fastq::String, bc_df::DataFrame, output_dir::String, match, mismatch, indel)
#     open(fastq, "r") do file
#         for line in eachline(file)
#             if line[1] == '@' && mode == "header"
#                 header = line
#                 mode = "seq"
